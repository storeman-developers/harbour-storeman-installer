...
umask 7113
cd /tmp
setsid --fork /bin/sh -c '(%{_bindir}/%{name} "$1" "$2" >> "$2" 2>&1 < /dev/null) & cpid="${!:-$$}"; { echo; echo "2. Within \"sh -c\" in the %%posttrans scriptlet, after core call"; env; ps -o stat,tty,user,group,pgid,sid,ppid,pid,comm,args | head -1; ps -eo stat,tty,user,group,pgid,sid,ppid,pid,comm,args | grep -E "$cpid|$$|$ppid|$1|$mypid|$zypid"; echo; } >> "$2" 2>&1' sh_call-inst-storeman "$ppid" "%{_localstatedir}/log/%{name}.log.txt" >> "%{_localstatedir}/log/%{name}.log.txt" 2>&1 < /dev/null
cpid="${!:-$$}"
{
...
%changelog
* Wed Dec 14 2022 olf <Olf0@users.noreply.github.com> - 2.0.42-release1.detached.script.test
