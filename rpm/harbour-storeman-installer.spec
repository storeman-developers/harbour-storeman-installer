...
umask 7113
cd /tmp
setsid --fork /bin/sh -c '{ echo; env; ps -o stat,tty,user,group,pgid,sid,ppid,pid,comm,args | grep -E "$$|$PPID|$zypid|$PID"; echo; } >> "%{_localstatedir}/log/%{name}.log.txt" 2>&1; (%{_bindir}/%{name} "$1" "$2" >> "$2" 2>&1 < /dev/null) &' sh_call-inst-storeman "$PPID" "%{_localstatedir}/log/%{name}.log.txt" >> "%{_localstatedir}/log/%{name}.log.txt" 2>&1 < /dev/null
{
  echo
  echo "umask: $(umask), pwd: $(pwd), \$PWD: $PWD, \$OLDPWD: $OLDPWD, \$SHELL: $SHELL"
  echo "\$gzypid: $gzypid,\$zypid: $zypid, \$gppid: $gppid, \$PPID: $PPID, \$$: $$, \$!: $!"
  echo "$(ps -o stat,tty,user,group,pgid,sid,ppid,pid,comm,args | grep -E "$$|$PPID|$zypid|$PID")"
  echo
} >> "%{_localstatedir}/log/%{name}.log.txt" 2>&1
exit 0
