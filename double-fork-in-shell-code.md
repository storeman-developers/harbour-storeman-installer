...
This is the generic form, the strictly static elements are the sequence:
`setsid --fork sh -c '(<cmd-list_or_script-call_which_does: kill $PPID>) &'`

### Variations
- The environment is copied down through the call chain with all shell implemetations (as usual), *except* across the `setsid --fork sh -c ' '` sequence, for which a fresh default environment might be provided (e.g., always, when a different shell executable is called by setsid than the callers shell).
  Hence the safe way to carry parameters across this sequence down the call chain are positional parameters, as described in the man-page of the shell of your choice for the option `-c`.
  Also note that all shells must copy their environment to sub-shells opened with `( )` and commands run in a sub-shell (what most shells do for every shell-external command), additionally some shells also copy their local variables down to sub-shells opened with `( )`, but this is implementation dependent.
- One can set the "inner" umask and PWD (via `cd`) as it fits best: These are just generic, most often used values; I often set the umask more restrictively.
  Note that the directory to change into must exist (you do not want a `cd` at this point to fail), hence `/` is a safe value, as e.g., `/tmp` is not available early in the boot-phase.
- One sure can redirect from or to anywhere else than `/dev/null` or redirect StdIN and StdERR differently.
- With a proper POSIX-compliant shell, one can close any channel with "-", e.g., for StdIN `>&-`, instead of redirecting it from or to `/dev/null`.
  But e.g., busybox's ash is not at all fully POSIX-compliant!
  Also note that when closing StdOUT or StdERR, anything writing to a closed channel will (/ might / should / must?  POSIX might tell.) fail, just as reading from a closed StdIN, in contrast to redirections to `/dev/null`.
...
- The explicit use of a sub-shell may be superfluous in some special cases: IMHO all shells start a single detached command (i.e., `<cmd> <args> <redirs> &`) automatically in a sub-shell, many execute all external commands in a sub-shell etc., but there may be cases in which this assumption will fall short.
...
- https://unix.stackexchange.com/questions/715248/double-fork-why
...
- https://subscription.packtpub.com/book/networking-and-servers/9781784396879/11/ch11lvl1sec73/double-fork-and-setsid
...
- https://gist.github.com/mynameisrufus/1372491#file-simple_daemon-rb-L2
  Another concise write-up, with example in Ruby.
- https://news.ycombinator.com/item?id=8355376
