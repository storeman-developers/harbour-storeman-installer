...
- The statement must be started detached (`setsid` does that implicitly, if *not* using the option `-w`: Hence appending a `&` is not necessary), otherwise the caller would be stopped during the execution of the statement, hence the caller would not be able to receive and handle a SIGTERM sent to it by `kill`.
